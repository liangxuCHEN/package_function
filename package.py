...
def output_res(all_rects, bin_width, bin_height):
    rects = list()
    all_positions = list()
    ...
    for s in all_positions:
        r = use_rate(s, bin_width, bin_height)
        total_rate += r
        rate_list.append(r)
...
def get_shape_data(shape_data, bin_data):
        """
...
class PackerSolution(object):
    ...
    """
    def __init__(self, rects_data, bins_data, border=0, bins_num=None):
        """
        ...
        """
        self._border = border
        self._data = get_shape_data(rects_data, bins_data)
        # TODO: bins paramet
    ...
    def _tidy_shape(self, bin_key):
        ...
        for shape in tmp_list:
            print type(shapes_num[index_shape])
            for num in range(0, shapes_num[index_shape]):
                new_list.append(shape)
            index_shape += 1
        return new_list, tmp_list, shapes_num

    def find_best_solution(self, all_shapes, bin_key, packer_id_list=None):
        """
        遍历各种算法找最佳的方案
    ...
    def find_solution(self, algo_list=None):
        if self.is_valid():
            ...
            for bin_key in self._data['data'].keys():
                ...
                best_solution, empty_positions, best_rate, best_packer = self.find_best_solution(
                    all_shapes, bin_key, packer_id_list=algo_list)
